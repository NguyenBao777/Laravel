...
if (selectChapter) {
    window.addEventListener("scroll", function () {
        const selectChapter = document.querySelector(".select-chapter-nav");
        if (window.scrollY > 200) {
            selectChapter.classList.add("p-fixed");
        } else {
            selectChapter.classList.remove("p-fixed");
        }
    });
...
$(document).ready(function () {
    if (localStorage.getItem("swich_theme") !== null) {
        const data = localStorage.getItem("swich_theme");
        const data_obj = JSON.parse(data);

        $(document.body).toggleClass(data_obj.class_1);
        $(".select-btn").toggleClass(data_obj.class_2);
        $(".menu-container").toggleClass("bg-dark bg-light");
        $(".menu-nav").toggleClass("navbar-dark navbar-light");
    }
    if (localStorage.getItem("list_book_like") == null) {
        localStorage.setItem("list_book_like", "[]");
    }
    //  Book Detail - Like btn
    if ($(".like-btn-container")) {
        const liSt_book_liked = localStorage.getItem("list_book_like");
        const list_book_liked_data = JSON.parse(liSt_book_liked);
        if (
            list_book_liked_data.some(
                (element) => element.id == $(".like_list_id").val()
            )
        ) {
            $(".like-btn-container")
                .append(`<button onclick={Like()} class="btn-like btn btn-danger btn-sm my-2">
                    <ion-icon name='checkmark-done-outline'></ion-icon>
                    <span>Đã thích</span> 
                </button>`);
            $(".like-btn-container").addClass("liked");
        } else {
            $(".like-btn-container")
                .append(`<button onclick={Like()} class="btn-like btn btn-danger btn-sm my-2">
...
$(".select-btn").on("click", function () {
    ...
    if ($(this).hasClass("checked")) {
        var isSelected = {
            class_1: "bg-dark text-white",
            class_2: "checked",
        };
        localStorage.setItem("swich_theme", JSON.stringify(isSelected));
    } else {
        localStorage.removeItem("swich_theme", JSON.stringify(isSelected));
    }
});

// Book Detail - function like book
function Like() {
    const id = $(".like_list_id").val();
    const name = $(".like_list_name").val();
    const img = $(".like_img").attr("src");
    const url = $(".like_list_url").val();
    const item = {
        id: id,
        name: name,
        img: img,
    ...
    var old_data = JSON.parse(localStorage.getItem("list_book_like"));
    var matches = $.grep(old_data, function (obj) {
        return obj.id == id;
    });
    if (matches.length) {
        old_data = old_data.filter((element) => {
    ...
    localStorage.setItem("list_book_like", JSON.stringify(old_data));
    renderListBook();
    //Render btn-like
    if (!$(".like-btn-container").hasClass("liked")) {
        $(".like-btn-container").children(".btn-like").remove();
        $(".like-btn-container")
            .append(`<button onclick={Like()} class="btn-like btn btn-danger btn-sm my-2">
        <ion-icon name='checkmark-done-outline'></ion-icon>
        <span>Đã thích</span> 
        ...
        $(".like-btn-container").removeClass("liked");
    }
}
...
function renderListBook() {
    ...
    if ($("#list_book_liked")) {
        $("#list_book_liked > div").remove();
        list_book_liked_data.forEach((element) => {
            $("#list_book_liked").append(`
                <div class="row mt-2" data-id="${element.id}">
                    <div class="col-md-5">
                        <img class="img img-responsive card-img-top" width="100%" src="${element.img}" alt=${element.class_1name}/>
                    </div>

                    <div class="col-md-7 sidebar">
                        <a href="${element.url}" class="text-decoration-none">
                            <p class="text-primary tex-sm-center">${element.name}</p>
                        </a>
                    </div>
                </div>
            `);
